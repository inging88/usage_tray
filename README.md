<<<<<<< HEAD
# usage_tray
Claude Code &amp; Codex 남은 토큰 작업표시줄 표기
=======
# usage-tray (Go) — Windows · macOS

Claude Code · Codex 사용량을 **트레이(Windows) · 메뉴바(macOS)** 에서 본다.
단일 실행 파일이고 설치 과정이 없다. 설정 파일도 없다 — 이미 로그인해 둔 자격증명을 읽는다.

PowerShell 판(상위 폴더)의 후속이다. 같은 데이터·같은 규칙을 쓰되 두 OS 에서 돈다.
...
| 툴팁·메뉴 | 두 에이전트의 7d/5h 남은량과 리셋 시각 |
...
>>>>>>> 0849847 (usage-tray: Claude Code · Codex 사용량을 트레이·메뉴바에서 본다)
